...
namespace xtrainer{
  ...
  address_t ForeignThread::stackPush(address_t basePointer, Argument arg){
  ...
  address_t ForeignThread::stackPop(address_t basePointer, size_t size, void* out){
  ...
  void ForeignThread::run_explicit(address_t foreignAddr, Argument ret, vector<Argument> args){
    ...
    address_t stack = parent->allocateMemory(STACK_SIZE, RW);
    custom.r<address_t>("rsp") = stack + STACK_SIZE;
    if(custom.r<address_t>("rsp") % 16)
      custom.r<address_t>("rsp") += 16 - ((size_t)custom.r<void*>("rsp") % 16);
    LOG << "Stack address: " << custom.r<address_t>("rsp") << "\n";
    ...
    for(int i = 0; i < args.size(); i++){
      ...
      else custom.r<address_t>("rsp") = stackPush(custom.r<address_t>("rsp"), args[i]);
    }

    //allocate parameter space (Windows x64)
    custom.r<address_t>("rsp") -= 0x30;

    //push 0 onto the stack as the return address to cause SIGSEGV
    size_t death = 0;
    custom.r<address_t>("rsp") = stackPush(custom.r<address_t>("rsp"), Argument((void*)&death, sizeof(void*)));

    custom.r<address_t>("rip") = foreignAddr;
    ...
    LOG << "Final PC value: " << std::hex << (size_t)custom.r<address_t>("rip") << "\n";
    LOG << "Final Stack value: " << std::hex << (size_t)custom.r<address_t>("rsp") << "\n";
